...
func (p *Porkbun) GetCurrentARecord() (string, error) {
	url := fmt.Sprintf("%s%s/A/%s", constants.RetrieveByNameType, p.config.Domain, p.config.Subdomain)
...
func (p *Porkbun) UpdateARecord(ip string) error {
	url := fmt.Sprintf("%s%s/A/%s", constants.EditByNameTypeURL, p.config.Domain, p.config.Subdomain)
